...
> Tip: `import SwiduxPaywall` is needed in *every* file that touches `store.paywall.*` or `PaywallAction` — including views that read the slice for display. `@_exported import Swidux` re-exports core Swidux only, not plugin modules. See <doc:PluginArchitecture>.
