...
There *is* real hardening around the honest path: reaching the attempt limit starts a cooldown, and during cooldown answers are refused even when correct, so the limit cannot be raced. None of that constrains a direct `.answerAccepted` dispatch — that is a bypass available to your own code, not to a user tapping the screen.
