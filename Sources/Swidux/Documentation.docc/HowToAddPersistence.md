...
- **`@Inline` blob columns** default to `Data()`, so any `Codable` type is CloudKit-safe through `@Inline`. Because `Data()` is never decodable, a **non-optional** `@Inline` property must also carry a domain default (`= …`) — the generated throwing getter uses it when the blob is empty (e.g. a row CloudKit materialized before the blob synced). Non-empty corrupt data throws, leaving the failed read visible instead of substituting a value that a later save could overwrite. Generated model construction and updates also throw when encoding fails; `EntityDB` rolls back the batch. Omitting the default is a compile-time error (`inlineRequiresDefault`).
